...
export class LogicService {
    ...
    private employee: BehaviorSubject<EmployeeInGroup[]> = new BehaviorSubject<EmployeeInGroup[]>(
        this.defaultListEmployee,
    );
    private accountant: BehaviorSubject<EmployeeInGroup[]> = new BehaviorSubject<EmployeeInGroup[]>(
        this.defaultListEmployee,
    );
    ...
    getAllWareHouse() {
        ...
        let subscription = this.warehouse.search(body).subscribe(
            (response: Warehouse[]) => {
                if (response) {
                    ...
                    this.snackbar.openSnackbar(
                        'Không thể tải danh sách kho hàng',
                        2000,
                        'Đóng',
                        'center',
                        'bottom',
    ...
    deleteWareHouse(list: string[]) {
        ...
        let sub = this.warehouse.deleteWareHouse(body).subscribe(
            (data) => {
                if (data) {
                    if (data.message > 0) {
    ...
    getAccountant(id: string) {
        ...
        if (id == 'root') {
            let sub = this.employeeService.GetAllEmployeePost('Kế toán', 1, 100).subscribe(
            ...
            let sub = this.employeeService.SearchEmployeeInGroupPost('Kế toán', id, 1, 100).subscribe(
    ...
    getEmployee(id: string) {
        this.employee.next([]);
        if (id == 'root') {
            let sub = this.employeeService.GetAllEmployeePost('Nhân viên', 1, 100).subscribe(
                (data: Response<Employee>) => {
                    if (data) {
                        let listEmployee: EmployeeInGroup[] = [];
                        data.data.forEach((e) => {
            ...
            let sub = this.employeeService.SearchEmployeeInGroupPost('Nhân viên', id, 1, 100).subscribe(
                (data: Response<EmployeeInGroup>) => {
                    if (data) {
    ...
    getUnitGroup() {
        this.unitGroup.next([]);
        let sub = this.employeeService.GetAllGroupByType(1).subscribe(
            (data) => {
                if (data) {
    ...
    addWarehouse(body: any) {
        let sub = this.warehouse.addWareHouse(body).subscribe(
            (data) => {
    ...
    updateWarehouse(body: any) {
        let sub = this.warehouse.updateWareHouse(body).subscribe(
            (data) => {
                if (data) {
    ...
    getWarehouse(id: string) {
        let sub = this.warehouse.getWareHouseById(id).subscribe(
            (data) => {
                if (data) {
                    this.detailWarehouse.next(data);
                } else {
                    this.snackbar.openSnackbar(
                        'Không tìm thấy thông tin kho hàng',
                        2000,
                        'Đóng',
                        'center',
                        'bottom',
    ...
    addAccountantToWarehouse(body: any) {
        let sub = this.warehouse.addAccountant(body).subscribe(
            (response) => {
                if (response && response.message == 1) {
                    this.snackbar.openSnackbar(
                        'Thêm kế toán vào kho hàng thành công',
                        2000,
                        'Đóng',
                        'center',
                        'bottom',
                    ...
                    this.snackbar.openSnackbar(
                        'Thêm kế toán vào kho hàng thất bại',
                        2000,
                        'Đóng',
                        'center',
                        'bottom',
    ...
    deleteAccountantToWarehouse(body: any) {
        let sub = this.warehouse.deleteAccountant(body).subscribe(
            (response) => {
                if (response && response.message == 1) {
                    this.snackbar.openSnackbar(
                        'Xóa kế toán ra khỏi kho hàng thành công',
                        2000,
                        'Đóng',
                        'center',
                        'bottom',
                    ...
                    this.snackbar.openSnackbar(
                        'Xóa kế toán ra khỏi kho hàng thất bại',
                        2000,
                        'Đóng',
                        'center',
                        'bottom',
