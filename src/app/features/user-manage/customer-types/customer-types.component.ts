...
export class CustomerTypesComponent implements OnInit {
    ...
    this.customerTypeService.getAllCustomerType(this.request).subscribe(
      (data) => {
        if (data) {
          ...
          this.loading = false;
          this.snackbar.openSnackbar('Không tìm thấy danh sách loại khách hàng', 2000, 'Đóng', 'center', 'bottom', false);
        }
      },
      (error) => {
        this.loading = false;
  ...
  DeleteCustomerTypes() {
    this.confirmService.openDialog({ message: 'Bạn có chắc chắn muốn xóa những loại khách hàng này?', confirm: 'Xác nhận', cancel: 'Hủy' }).subscribe(data => {
      if (data) {
  ...
  deleteCustomerTypesByIds(selectedIds: string[]) {
    ...
    let sub = this.customerTypeService.del(body).subscribe(data => {
      if (data && data.message > 0) {
        this.snackbar.openSnackbar('Xóa loại khách hàng thành công', 2000, 'Đóng', 'center', 'bottom', true);
        this.filter();
      } else {
