...
export class RouteService {
    ...
    SearchAllRoute(page: any, pageSize: any, groupId: any, employeeId: any, keyword: any): Observable<any> {
        ...
        // .pipe(map((response: any) => response));
        return this.http.get(this.urlAPIRoute + '/getall?' + params).pipe(map((response: any) => response));
    }

    GetRouteById(id: string): Observable<any> {
        return this.http.get(this.urlAPIRoute + '/id?Id=' + id).pipe(map((response: any) => response));
    }

    AddRoute(body: any): Observable<any> {
        return this.http.post(this.urlAPIRoute + '/add', body).pipe(map((response: any) => response));
    }

    UpdateRoute(body: any): Observable<any> {
        return this.http.put(this.urlAPIRoute + '/update', body).pipe(map((response: any) => response));
    }

    SearchAllCusInRoute(body: any): Observable<any> {
        return this.http.post(this.urlAPIRoute + '/searchAllCusInRoute', body).pipe(map((response) => response));
    }

    CountCustomerInRoute(routeId: any): Observable<any> {
        let params = new HttpParams().set('RouteId=', routeId);
        return this.http.get(this.urlAPIRoute + '/CountCustomerInRoute?', { params }).pipe(map((res) => res));
    }

    DeleteCusFromRoute(body: any): Observable<any> {
        return this.http.delete(this.urlAPIRoute + '/deleteCusFromRoute?', { body }).pipe(map((res) => res));
    }
    Import(body: any): Observable<any> {
        return this.http.post(this.urlAPIRoute + '/Import', body).pipe(map((res) => res));
    }
    Delete(body: any): Observable<any> {
        return this.http.post(this.urlAPIRoute + '/Delete', body).pipe(map((res) => res));
    }
    ...
    export(type: number, data: any): Observable<any> {
        ...
        return this.http.post(this.urlAPIRoute + '/export', body, { responseType: 'blob' });
    }
}
